...
function webpackConfig(moduleName, basePath) {
  ...
  const externals = [
    ...
    // 'react-i18next' intentionally bundled via alias shim to avoid PropTypes crash
    'react-layout-pane', 'react-redux', 'react-select',
  ...
  return {
    ...
    resolve: {
      ...
      alias: {
        'react-i18next': path.join(basePath, 'src', 'shims', 'react-i18next.ts'),
        ...alias,
      },
    },
    devtool: 'source-map',
...
const config = webpackConfig('mo-import', __dirname);
